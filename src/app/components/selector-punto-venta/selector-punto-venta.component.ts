...
import { SessionUsuario } from 'src/app/shared/models/usuario.model';
...
export class selectorPuntoVentaComponent implements OnInit {
                ...
                next: (res) => {
                    ...
                    /*if (this.item.idSucursal > 0) {
                        ...
                        if (this.listaSucursales.length==1 && this.listaPuntoVenta.length==1){
                         ...
                         // verificar si es la unica sucursal y el unico pto venta
                         if (this.listaSucursales.length==1 && this.listaPuntoVenta.length>0 && this.sessionService.getSessionUserData().codigoTipoUsuario===adm.TIPO_USUARIO_ASISTENTE){
                            this.itemForm.patchValue({ idSucursal: this.listaSucursales[0].id });
                            this.itemForm.patchValue({ idPuntoVenta: this.listaPuntoVenta[0].id });
                            this.establecerPunto();
                        }
    ...
    establecerPunto():void{
        ...
        this.mensajeService.showSuccess('Punto de Venta establecido');
        this.verificarTurno();
    ...
    verificarTurno() {
        ...
        const busquedaTurno: any = {
            ...
            idEstadoTurno: adm.ESTADO_TURNO_ABIERTO,
        ...
        this.turnoService.get(busquedaTurno).subscribe({
            next: (res) => {
                ...
                if (
                    this.sessionService.getSessionUserData().codigoTipoUsuario ==
                    adm.TIPO_USUARIO_ASISTENTE
                ) {
